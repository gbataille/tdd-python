...
class ItemModelTest(TestCase):
    ...
    def test_list_ordering(self):
        list1 = List.objects.create()
        item1 = Item.objects.create(list=list1, text='first item')
        item2 = Item.objects.create(list=list1, text='second item')
        item3 = Item.objects.create(list=list1, text='third item')
        self.assertEqual(list(Item.objects.all()), [item1, item2, item3])

    def test_string_representation(self):
        list_ = List.objects.create()
        item = Item(list=list_, text='An item')
        self.assertEqual(str(item), 'An item')
